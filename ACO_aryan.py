...
class Node:
    def __init__(self, node_id, x, y, energy, n, max_num_ants):
        ...
        self.pheromone_matrix = np.full((n, n), 0.5)    # Pheromone matrix (n x n)
    ...
    def generate_ant(self, source_node, sink_node, pheromone_matrix):
        """
...
def initialize_nodes(n, side_length, r, initial_energy, max_num_ants=5):
    ...
    source_node = Node(node_id=0, x=0, y=0, energy=initial_energy, n=n, max_num_ants=max_num_ants)  # Infinite energy for sink
    positions.add((0, 0))
    ...
    while len(nodes) < n:  # Leave one position for sink node
    ...
    for node in nodes:
        for other_node in nodes:
            if node.node_id != other_node.node_id:
                ...
                if distance <= r:
                    node.add_neighbor(other_node.node_id)
...
def initialize_ants(source_node, nodes, sink_node):
    nodes[source_node].generate_ant(source_node,sink_node,nodes[source_node].pheromone_matrix)
...
def get_next_hop_probabilities(node, ant, nodes, alpha=1.0, beta=2.0, m=0.5):
    ...
    source_node = nodes[ant.source_node] #Source node
...
def establish_route(source_node_id, sink_node_id,  nodes):
    """
    ...
    evaporate_pheromones(nodes[source_node_id])
    add_pheromone_from_ants(nodes[source_node_id], Q=1.0)

    # Initialize ants for the next round
    nodes[source_node_id].initialize_k_ants()
...
# establish_route(source_node.node_id, sink_node.node_id, nodes)
establish_route(source_node.node_id, sink_node.node_id, nodes)
